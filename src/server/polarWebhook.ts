import { validateEvent, WebhookVerificationError } from "@polar-sh/sdk/webhooks";

export const polarWebhookRoute = {
  async action({ request }: { request: Request }) {
    // Read raw body as ArrayBuffer to preserve signature integrity
    const bodyBuffer = await request.arrayBuffer();

    const secret = process.env.POLAR_WEBHOOK_SECRET ?? "";
    if (!secret) {
      return new Response("Webhook secret missing", { status: 500 });
    }

    let event: any;
    try {
      event = validateEvent(Buffer.from(bodyBuffer), request.headers, secret);
    } catch (err: any) {
      if (err instanceof WebhookVerificationError) {
        return new Response("Invalid signature", { status: 403 });
      }
      console.error("[polarWebhookRoute] validation error", err);
      return new Response("Webhook error", { status: 400 });
    }

    try {
      // Handle relevant events – extend later
      switch (event.type) {
        case "order.paid":
          // TODO: credit user in DB based on external_customer_id
          break;
        case "order.refunded":
          // TODO: adjust credits / mark refund
          break;
        case "customer.state_changed":
          // TODO: sync subscription status
          break;
        default:
          // Ignore other events for now
          break;
      }
      return new Response("ok", { status: 202 });
    } catch (err: any) {
      console.error("[polarWebhookRoute] handler error", err);
      return new Response("Server error", { status: 500 });
    }
  },
}; 